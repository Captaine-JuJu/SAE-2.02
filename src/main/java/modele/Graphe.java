...
public class Graphe {
    ...
    public ArrayList<Integer> degreEntrantZero(){
        ...
        for (int sommet : chMapVoisins.keySet()){
            if(degreeEntrant().get(sommet).equals(0)){
    ...
    public ArrayList<Integer> triToploogique(){
        ...
        TreeMap<Integer, Integer> degresEntrant = degreeEntrant();
        while (!sources.isEmpty()){
            int source = sources.remove(0);
            resultat.add(source);
            for (int voisin : chMapVoisins.get(source)){
